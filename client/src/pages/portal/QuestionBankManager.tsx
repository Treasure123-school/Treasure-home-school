...
function ContextFilterBar({
  ...
  return (
    <div className="space-y-4">
          ...
          >
            <SelectTrigger data-testid="ctx-select-subject" className="h-9">
              <SelectValue placeholder={
                ...
                classSubjects.length === 0 ? "No subjects" :
                                           "Select subject…"
              } />
            </SelectTrigger>
            <SelectContent>
      ...
      {(!value.classId || !value.subjectId || !value.termId) && (
        <div className="flex items-start gap-2 bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/50 rounded-lg px-3 py-2.5">
          ...
          <p className="text-xs text-blue-700 dark:text-blue-300">
            Select <strong>Class → Subject → Term</strong>{showBank ? " → Bank" : ""} to load questions. 
            This prevents slow page loads and keeps questions scoped to the right context.
          </p>
        </div>
      )}
    </div>
